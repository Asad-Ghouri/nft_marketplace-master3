...
const StakingMutant: NextPage = () => {
    ...
    const stakingContractAddress = "0x9D2D05873665572c049D34b2Ba0AfD1D651266FB";
    ...
    const { mutateAsync: startEarning, isLoading: EarnLoad } = useContractWrite(contract, "startEarning")

    const call = async () => {
        try {
            const data = await contract.call("startEarning")
    ...
    useEffect(() => {
        ...
        async function loadClaimableRewards() {
            const stakeInfo = await contract?.call("getReward", address);
            // const data = await contract.call("claimRewards")
            setClaimableRewards(stakeInfo._hex);
            console.log(stakeInfo._hex)
        }
    ...
    return (
        <>
            ...
            <img
                ...
                alt=""
            />
            <h3 className="user-frs swap-frs">CLAIM NANA</h3>
